...
#define DEF_CONFIG              "home/config.txt"                    /** Default config filename. */
#define ARG_PROFILE             "--profile"                     /** Flag to override the profile file. */
#define DEF_PROFILE             "home/profile.txt"                   /** Default profile filename. */
...
/* PLUM edit start */
#define BUTTON_LABEL_PLUM_MODE  "M"                             /** Ascii text fallback for the plum mode label. */
#define BUTTON_LABEL_REFRESH    "R"                             /** Ascii text fallback for the refresh label. */
#define BUTTON_LABEL_HELP    	"?"                             /** Ascii text fallback for the help label. */
#define BUTTON_LABEL_SELECTALL  "Select All"                    /** Ascii text fallback for the select all label. */
/* PLUM edit finish */

#define BUTTON_LABEL_CONFIG     "Options"                       /** Ascii text fallback for the options button label. */
#define BUTTON_LABEL_EDIT       "Edit Item"                     /** Ascii text fallback for the edit item button label. */
...
enum MODES_T {
    ...
    /* PLUM edit start */
	MODE_PLUM_BROWSE,    	/** browse active media server */
	MODE_PLUM_SEARCH,		/** Look for media servers */
	MODE_PLUM_HELP,			/** Help screen to explain icon and key usage */
	
	// PLUM doesn't make use of the ARGUMENT / VALUE / OPTION modes
...
class CSelector : public CBase
{
    ...
    private:
        ...
        // PLUM doesn't include zip support
        ...
        /** @brief Load the display list with items with launcher options. (NOT COMPLETED)
         */
        //void    PopModeOption       ( void );
        
        /** @brief Load the display list with the media servers we've discovered
         */
        void	PopModePlumSearch	( void );
        
        /** @brief Load the display list with contents from active media server
         */
        void	PopModePlumBrowse	( void );
        
        /** @brief Load the display list with help for keyboard / icon usage
         */
        void	PopModePlumHelp		( void );
        /* PLUM edit finish */
        ...
        /* PLUM edit start */
        // PLUM doesn't make use of Argument / Value / Option
        ...
        /* PLUM edit start */
		CPlum					Plum;					/**< Class with methods and data needed for browsing media servers */
		string					HelpImagesDirectory;    /**< Directory containing images showing what icons or keys to press */
		/* PLUM edit finish */
};
